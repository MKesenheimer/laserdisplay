...
| `translate_to` | moves the shape's center to a target position and leaves it there: either `sx`, `sy` (start point) and `ex`, `ey` (end point, both required) — the center moves from start to end in `dt` seconds (default 1.0), easing in and out — or `path` (name of another defined shape, never created): the center follows the path from its first to its last point in `dt` seconds (a closed path leads to a full lap) |
| `rotate_by` | required `angle` (degrees, negative = the other way); rotates the shape by that angle in `dt` seconds (default 1.0) and leaves it in the final orientation, easing in and out; the rotation happens around `px`, `py` (both or neither) — without a pivot the shape rotates around its own center |
